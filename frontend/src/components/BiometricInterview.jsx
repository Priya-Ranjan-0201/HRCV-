...
const BiometricInterview = ({ isOpen, onClose }) => {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      if (!isReady) {
        setErrorMsg('HRCV server is taking longer than expected to wake up. Please try again.');
        return;
      }
      ...
      if (currentQuestionIndex < questions.length - 1) {
        ...
        localStorage.setItem('hrcv_latest_interview', JSON.stringify(payload));
        localStorage.setItem('tonycv_latest_interview', JSON.stringify(payload));
        setShowResults(true);
      }
    } catch (err) {
      ...
      if (currentQuestionIndex < questions.length - 1) {
        ...
        localStorage.setItem('hrcv_latest_interview', JSON.stringify(fallbackPayload));
        localStorage.setItem('tonycv_latest_interview', JSON.stringify(fallbackPayload));
        setShowResults(true);
      }
    } finally {
